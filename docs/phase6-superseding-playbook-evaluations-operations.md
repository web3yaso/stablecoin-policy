...
Do not apply `0035` before the existing Phase 6 receiver cutover is complete.
The required order remains migrations `0031`, `0032`, `0033`, `0034`, then
`0035`.
...
3. Run a linked migration dry-run through `0035`, database lint, and the full
   pgTAP suite.
4. Apply migration `0035` and deploy the application.
...
- production migrations `0031`–`0035`, receiver configuration, deployment,
  and signed smoke were not performed by this development checkpoint.
