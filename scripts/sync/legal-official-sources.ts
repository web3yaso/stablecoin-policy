...
async function main() {
  ...
  const selected = requestedId
    ...
    : sources;
  if (selected.length === 0) throw new Error(`unknown source: ${requestedId}`);
  ...
  for (const source of selected) {
    if (source.provider !== "eur-lex") throw new Error(`unsupported source provider: ${source.provider}`);
    const snapshot = await fetchEurLexSource(source);
    console.log(
